...
from app.keyboards.admin import build_admin_broadcast_preview_keyboard, build_admin_main_menu_keyboard
...
def _extract_content(message: Message) -> BroadcastContent | None:
    if message.photo:
        return BroadcastContent(BroadcastContentType.PHOTO, message.caption, message.photo[-1].file_id)
    if message.video:
        return BroadcastContent(BroadcastContentType.VIDEO, message.caption, message.video.file_id)
    if message.animation:
        return BroadcastContent(BroadcastContentType.ANIMATION, message.caption, message.animation.file_id)
    if message.document:
        return BroadcastContent(BroadcastContentType.DOCUMENT, message.caption, message.document.file_id)
    if message.text:
...
async def _resend_preview(message: Message, content: BroadcastContent) -> None:
    ...
    if content.content_type == BroadcastContentType.TEXT:
        ...
        await message.answer_photo(content.file_id, caption=content.text, reply_markup=keyboard)
    elif content.content_type == BroadcastContentType.VIDEO:
        await message.answer_video(content.file_id, caption=content.text, reply_markup=keyboard)
    elif content.content_type == BroadcastContentType.ANIMATION:
        await message.answer_animation(content.file_id, caption=content.text, reply_markup=keyboard)
    elif content.content_type == BroadcastContentType.DOCUMENT:
        await message.answer_document(content.file_id, caption=content.text, reply_markup=keyboard)
...
async def handle_broadcast_start(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if callback.message is not None:
        await callback.message.edit_text("Отправьте текст, фото, видео, GIF или документ для рассылки.")
...
async def handle_broadcast_content(message: Message, state: FSMContext) -> None:
    ...
    if content is None:
        await message.answer("Не удалось распознать содержимое. Отправьте текст, фото, видео, GIF или документ.")
        return
    await state.update_data(
        content_type=content.content_type.value, content_text=content.text, content_file_id=content.file_id
    )
...
async def handle_broadcast_restart(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.answer()
    if callback.message is not None:
        await callback.message.answer("Отправьте текст, фото, видео, GIF или документ для рассылки.")
...
async def handle_broadcast_cancel(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if callback.message is not None:
        await callback.message.answer("⚙ Панель управления", reply_markup=build_admin_main_menu_keyboard())
...
async def handle_broadcast_send(
    ...
    content = BroadcastContent(
        BroadcastContentType(data["content_type"]), data.get("content_text"), data.get("content_file_id")
    )
    await state.clear()
    await callback.answer("Рассылка запущена…")

    result = await BroadcastService(callback.bot, session).send(content)
    await AdminAuditLogRepository(session).add(
        user.telegram_id, "broadcast_sent", None, f"sent={result.sent} blocked={result.blocked} failed={result.failed}"
    )
