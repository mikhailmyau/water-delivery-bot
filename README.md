...
Коммерческий Telegram-магазин доставки питьевой воды: выбор одного из трёх
видов воды и объёма (счётчик бутылей), выбор города доставки с автоматической
оценкой срока (1/3/5 дней), оплата через абстрактный платёжный провайдер
...
Бот продаёт три вида воды (`app/database/models/water_type.py::WaterType`):
СЛМ (слабоминерализованная негаз.), СРМ (среднеминерализованная негаз.) и
ГАЗ (газированная). У каждого — своя цена за литр в `bot_settings`
(`price_slm_per_liter`, `price_srm_per_liter`, `price_gaz_per_liter`),
администратор меняет их через `/price`, применяется мгновенно, без
перезапуска бота. Названия и описания видов — в `app/content.py`.

Объём заказа считается бутылями по 20 л (`BOTTLE_VOLUME_LITERS` в
`app/utils/constants.py`): от `MIN_BOTTLES` (2, то есть 40 л) до
`MAX_BOTTLES` (10, то есть 200 л). Свободный ввод объёма и добавление новых
видов воды через интерфейс не предусмотрены намеренно — при необходимости
добавить четвёртый вид воды: добавьте значение в `WaterType`, колонку цены в
`BotSettings` + миграцию, и запись в `WATER_TYPE_LABELS`/`_DESCRIPTIONS` в
`app/content.py`.
...
Название компании, слоган, ссылки на канал с поставками/группу отзывов,
текст гарантий и подписи видов воды — всё собрано в одном файле,
`app/content.py`, с комментариями "ЗАПОЛНИТЬ" там, где сейчас стоят
заглушки. Замените их перед продакшен-запуском: имя компании, реальные
t.me-ссылки и формулировки гарантий должны отражать то, что вы реально
предлагаете клиентам.

Вступительный абзац приветствия (`bot_settings.welcome_text`) редактируется
администратором прямо в БД или через будущий пункт `/admin` — цены и
гарантии дорисовываются поверх него автоматически при каждом показе
